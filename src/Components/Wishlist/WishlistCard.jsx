...
const WishlistCard = ({ product, setLatestData, latestData }) => {
  ...
  const handleRemoveWishlist = async () => {
    await axios
      .patch("https://quick-ponno-server.vercel.app/wishlist/remove", {
        userEmail: userEmail,
  ...
  return (
    <div className="flex items-center gap-2">
      <div>
        <img className="w-32 border-2 rounded-xl p-2" src={image} alt="" />
      </div>
      ...
      <h1 className="w-60">
        {" "}
        <span className="font-semibold">Price</span> <br />${price}
      </h1>
      <h1 className="w-60">
        {" "}
        <span className="font-semibold">Items Left</span> <br />
        {stock}
      </h1>
      <button
